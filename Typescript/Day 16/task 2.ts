...
const readPackage = (input: string): Package  => {
  ...
  if (packet.typeId === 4) {
    ...
    switch(packet.typeId) {
      case 0: 
        packet.value = packet.subPackages.reduce((sum, p) => sum + p.value, 0);
        break;
      case 1:
        packet.value = packet.subPackages.reduce((product, p) => product * p.value, 1);
        break;
      case 2:
        packet.value = packet.subPackages.reduce((min, p) => Math.min(min, p.value), Math.pow(2,64));
        break;
      case 3:
        packet.value = packet.subPackages.reduce((max, p) => Math.max(max, p.value), -1);
        break;
      case 5: 
        packet.value = packet.subPackages.length == 2 && packet.subPackages[0].value !== undefined && packet.subPackages[1].value !== undefined ?  packet.subPackages[0].value > packet.subPackages[1].value ? 1 : 0 : NaN;
        break;          
      case 6: 
        packet.value = packet.subPackages.length == 2  && packet.subPackages[0].value !== undefined && packet.subPackages[1].value !== undefined ? packet.subPackages[0].value < packet.subPackages[1].value ? 1 : 0 : NaN;
        break;          
